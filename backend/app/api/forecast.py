...
def get_sku_forecast(
    ...
    if not sales or len(sales) < 14:
        raise HTTPException(
            status_code=400,
            detail="Need at least 14 daily sales records to generate a forecast.",
        )
    ...
    # Combine last 14 days of actual sales + future forecast
    ...
    rec = (
        ...
        .filter(RecommendationRecord.product_id == product.id)
        .first()
    )
