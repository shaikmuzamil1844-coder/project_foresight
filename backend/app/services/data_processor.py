...
try:
    from backend.app.models.db_models import (
        Product, SalesRecord, InventoryRecord, ForecastRecord, RecommendationRecord,
    )
...
class DataProcessor:
    ...
    def ingest_dataframe(df: pd.DataFrame, db: Session) -> int:
        """
        Ingest a CSV/XLSX DataFrame into the database.
        Returns the number of unique SKUs processed.
        """
        ...
        for _, row in unique_skus.iterrows():
            lead_time = 7
            if "supplier_lead_time" in df.columns:
                lt_val = df[df["sku_id"] == row["sku_id"]]["supplier_lead_time"].iloc[0]
                if pd.notna(lt_val):
        ...
        # ── 2. Clear stale records for a clean re-seed ───────────────────────
        ...
        # ── 3. Bulk-insert Sales + Inventory rows ────────────────────────────
        sales_records     = []
        inventory_records = []
        ...
        for _, row in df.iterrows():
            ...
            price = float(row["price"])
            stock = int(row["inventory"]) if "inventory" in row and pd.notna(row.get("inventory")) else 50
        ...
        # ── 4. Risk Matrix + Initial Forecasts per SKU ───────────────────────
        forecaster = DemandForecaster()

        for prod in products:
            prod_sales = (
                db.query(SalesRecord)
                .filter(SalesRecord.product_id == prod.id)
            ...
            # Generate initial 30-day forecast (optional – skip if not enough data)
            if len(prod_sales) >= 14:
                df_prod = pd.DataFrame([
                    {"date": s.date, "units_sold": s.units_sold} for s in prod_sales
                ])
                try:
                    f_res = forecaster.train_and_forecast(df_prod, horizon_days=30)
                    ...
                    print(f"Initial forecast skipped for {prod.sku_id}: {e}")
